...
@pytest.mark.django_db
def test_send_kotokorent_fail(fkontokorent_payment):
    assert m.send(fkontokorent_payment.get('payment'),fkontokorent_payment.get('account'),fkontokorent_payment.get('currency'))==False
@pytest.mark.django_db
def test_send_fail(fpayment):
    assert m.send(fpayment.get('payment'),fpayment.get('account'),fpayment.get('currency'))==False
@pytest.mark.django_db
def test_recieve_ok(payment):
	assert m.recieve(payment.get('payment'),payment.get('account'),payment.get('currency'))==True
@pytest.mark.django_db
def test_recieve_ok2(fpayment):
    assert m.recieve(fpayment.get('payment'),fpayment.get('account'),fpayment.get('currency'))==True
@pytest.mark.django_db
def test_delete_history(payment):
    assert payment.get('account').history.count()==1
    assert m.delete_history(payment.get('account'))==True
